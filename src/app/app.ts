...
export class App {
  ...
  users = this.httpClient.get<{ users: User[] }>('https://dummyjson.com/users').pipe(map(({ users }) => users))

  usersPaginated: SelectHandler<User> = ({ pageNumber, pageCapacity }) => {
    return this.httpClient.get<{ users: User[], limit: number, skip: number, total: number }>
      (`https://dummyjson.com/users?limit=${pageCapacity}&skip=${pageNumber * pageCapacity}`)
      .pipe(map(({ users, limit, skip, total }) => {
        return {
          options: users,
          metadata: {
